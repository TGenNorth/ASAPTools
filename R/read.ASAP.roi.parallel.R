#' @title Read ASAP Region of Interest
#'
#' @description
#' UNDER DEVELOPMENT This function uses a user provided ASAP results XML path to parse sample and assay information from the ASAP output and display the SNPs.
#' @param XML A path with the ASAP XML file to parse.
#' @return An object with data from the ASAP xml.
#' @export read.ASAP.roi.parallel
#' @import xml2
#' @import tidyverse
#' @import foreach
#' @import doParallel
...
###UNDER DEVELOPMENT
#
# XML <- "/scratch/tporter/RSV_20230825_ROI_Formatting/RSV_NovaSeq0008_NEB_SQL/RSV_NovaSeq0008_NEB_SQL_analysis.xml"
# cores <- parallelly::availableCores()
#
# read.ASAP.roi.parallel <- function(XML, cores){
#
#   library(doParallel)
#   library(foreach)
#
...
#         )
#
#         for (ROI in 1:xml_length(Amplicon_Node)) {
#           #SNP = 1
#           #Get a single amplicon node
#           ROI_Node <- xml_child(Amplicon_Node, ROI)
#
#           if (xml_name(ROI_Node) == "region_of_interest"){
#
#             xml_attrs(ROI_Node)
#
#             xml_children(ROI_Node)
#
#             if(xml_length(ROI_Node) == 2){
#               #Get and format the SNP_Distirbution Data
#               SNP_Dist = as.character(xml_child(ROI_Node, 2))
#               SNP_Dist = str_remove(ROI_Dist, "<base_distribution ")
